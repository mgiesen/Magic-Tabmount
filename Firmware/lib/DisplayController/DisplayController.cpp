...
{
    enable = !enable; // Zustand aus semantischen Gruenden umkehren
...
void DisplayController::setActivationState(bool active)
{
    // Nur den internen Zustand setzen, KEINE Motorbewegung. Wird beim Boot
    // genutzt, um den zuletzt persistierten Zustand wiederherzustellen.
    _isActive = active;
}
